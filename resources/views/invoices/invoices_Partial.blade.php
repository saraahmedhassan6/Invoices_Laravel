...
@section('title')
    {{__('invoices.partially_paid_invoices')}}
@stop
@section('css')
...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">{{__('invoices.invoices')}}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ الفواتير
    {{__('invoices.partially_paid_invoices')}}
                </span>
            </div>
        </div>
...
@section('content')
    ...
    @if (session()->has('Status_Update'))
        <script>
            window.onload = function() {
                notif({
                    msg: "{{__('invoices.invoice_msg_update_succ')}}",
                    type: "success"
                })
            }
                ...
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <a href="invoices/create" class="modal-effect btn btn-sm btn-primary" style="color:white"><i
                                class="fas fa-plus"></i>&nbsp; {{__('invoices.invoice_adding')}}</a>
                    </div>
                </div>
                <div class="card-body">
                                                ...
                                                @can('ارشفة الفاتورة')
                                                    <a class="dropdown-item" href="#" data-invoice_id="{{ $invoice->id }}"
                                                       data-toggle="modal" data-target="#Transfer_invoice"><i
                                                        ...
                                                        {{__('invoices.invoice_move_archive')}}
                                                    </a>
                                                @endcan
    ...
    <div class="modal fade" id="Transfer_invoice" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{__('invoices.archived_invoice')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                    <form action="{{ route('invoices.destroy', 'test') }}" method="post">
